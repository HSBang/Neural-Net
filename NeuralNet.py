...
class NeuralNet:
    ...
    def __init__(self, input_dim, output_dim, epsilon):
        ...
        We believe that 4 hidden layer nodes are good enough to process train 
        the network. For our implementation with no hidden layers, you can set the
        number of nodes in the hidden layer to 1 (this is just so that the parameters
        that set a random number between 1 and the number of hidden nodes will function
        correctly.)
        """
        numHiddenNodes = 4
        
        self.Weight1 = np.random.randn(input_dim, numHiddenNodes) / np.sqrt(input_dim)
        self.bias1 = np.zeros((1, numHiddenNodes))
        self.Weight2 = np.random.randn(numHiddenNodes, output_dim) / np.sqrt(numHiddenNodes)
...
if linear:
    #load data
    X = np.genfromtxt('C:\Users\omaaa\Desktop\Lab4_Soln\DATA\ToyLinearX.csv', delimiter=',')
    y = np.genfromtxt('C:\Users\omaaa\Desktop\Lab4_Soln\DATA\ToyLineary.csv', delimiter=',')
    y = y.astype(int)
    #plot data
...
#B. Non-linearly separable data
    ...
    #load data
    X = np.genfromtxt('C:\Users\omaaa\Desktop\Lab4_Soln\DATA\ToyMoonX.csv', delimiter=',')
    y = np.genfromtxt('C:\Users\omaaa\Desktop\Lab4_Soln\DATA\ToyMoony.csv', delimiter=',')
    y = y.astype(int)
    #plot data
    plt.scatter(X[:,0], X[:,1], s=40, c=y, cmap=plt.cm.Spectral)
    plt.show()
